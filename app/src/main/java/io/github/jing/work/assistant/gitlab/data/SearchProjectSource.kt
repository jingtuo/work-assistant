 ...
 * 搜索工程
 */
class SearchProjectSource(private val service: Gitlab, private val search: String) :
    ...
    override fun loadSingle(params: LoadParams<Int>): Single<LoadResult<Int, Project>> {
        ...
        return service.searchProjects(search, page, params.loadSize)
            .subscribeOn(Schedulers.io())
            .map { data -> toLoadResult(page, data, params.loadSize) }
            .onErrorReturn {
                Log.e(MergeRequestSource.TAG, it.message, it)
                LoadResult.Error(it)
            }
    }
